...
# Substitute: cooperative-hunting, coworld-cooperative-hunting, 6.
#
...
#
# env:
#   SMOKE_IMAGE                image, if not given as $1        (coworld-cooperative-hunting:ci)
#   SMOKE_SLUG                 game slug                        (cooperative-hunting)
#   SMOKE_GAME_BIN             game entrypoint                  (/bin/cooperative-hunting)
#   SMOKE_PLAYER_BIN           player entrypoint                (/bin/cooperative-hunting-player)
...
image="${1:-${SMOKE_IMAGE:-coworld-cooperative-hunting:ci}}"
slug="${SMOKE_SLUG:-cooperative-hunting}"
...
# EVERY player container must also exit 0. The starter smoke only checked the
# game, so a player whose receive loop raises on a closed socket (whisky's
# receiveMessage raises on a close frame; mummy's send only queues, so the
# game's quit(0) can outrun the flushed frame) went green here and failed
# hosted certification with player_error intermittently -- raid 0.1.3 -> 0.1.4.
player_failures=0
for ((slot = 0; slot < seats; slot++)); do
  # Give a player that is still draining the socket a moment to notice the
  # game is gone; the game itself has already exited, so this is bounded.
  for _ in $(seq 1 20); do
    docker ps -q --filter "name=${prefix}-p${slot}" | grep -q . || break
    sleep 1
  done
  if docker ps -q --filter "name=${prefix}-p${slot}" | grep -q .; then
    echo "FAIL: player ${slot} was still running 20s after the game exited" >&2
    player_failures=1
    continue
  fi
  pcode="$(docker inspect -f '{{.State.ExitCode}}' "${prefix}-p${slot}")"
  if [ "${pcode}" != "0" ]; then
    echo "FAIL: player container ${slot} exited ${pcode} (must exit 0)" >&2
    player_failures=1
  fi
done
